...
cur_cds_PiD <- cur_cds[, rownames(pData(NucSeq.atac_cds)[pData(NucSeq.atac_cds)[[opt$clusters]] == cluster & pData(NucSeq.atac_cds)$Diagnosis == 'PiD',])]
...
print(paste('PiD CDS dimensions:', dim(cur_cds_PiD)))
...
cur_cicero_cds_PiD <- make_cicero_cds(cur_cds_PiD, reduced_coordinates = reducedDims(cur_cds_PiD)$UMAP)
...
saveRDS(cur_cicero_cds_PiD, file=paste0(opt$outdir,'/',cluster,'_cicero_cds_PiD.rds'))
...
conns_PiD <- run_cicero(cur_cicero_cds_PiD, human.hg38.genome, window = 3e+05, sample_num = 200)
...
# save(conns, conns_PiD, conns_control, file=paste0(opt$outdir,'/',cluster,'_cicero_connections.rda'))
save(conns_PiD, conns_control, file=paste0(opt$outdir,'/',cluster,'_cicero_connections.rda'))
